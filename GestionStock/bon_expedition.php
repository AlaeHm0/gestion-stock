...
$id = $_GET['id'];
$result = mysqli_query($conn, "SELECT * FROM sortie WHERE facture = '$id' GROUP BY facture");
...
function Info_societe_client($row_forn) {
...
function info_facture($id, $date_livraison, $client){
    $this->SetFont('Arial', 'B', 12);
    ...
    $this->Cell(10);
    $this->Cell(45,10,'Num client',1,0,'C');
    $this->Cell(45,10,$client,1,1,'C');
    //
    $this->SetFont('Arial', '', 12);
    $this->Cell(25,10,$date_livraison,1,0,'C');
    $this->Cell(15,10,'Devis',1,0,'C');
    $this->Cell(25,10,$id,1,0,'C');
    $this->Cell(25,10,$this->PageNo().'/{nb}',1,0,'C');
    $this->Cell(10);
    $this->Cell(90,10,'Objet: Bon de Expedition',1,1,'C');
    $this->Ln(10);
    //
}
function Signature(){
...
$result = mysqli_query($conn, "SELECT * FROM client WHERE id = '$client'");
$row_forn = mysqli_fetch_assoc($result);
$pdf->Info_societe_client($row_forn);
$pdf->info_facture($id, $date_livraison, $client);
// Main Content
$result = mysqli_query($conn, "SELECT 
pr.code as code,
pr.nom as nom,
ct.nom as categorie,
SUM(rc.quantite) as quantite
FROM sortie rc
...
$pdf->Signature();
$pdf->Output('bon_sortie.pdf', 'D');
header("Location: detail_expedition.php");
?>